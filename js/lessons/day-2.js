...
window.__lessonCache[2] = {
  ...
  "questions": [
    {
      "q": "Mobile team says they need 'just product name and price', but web needs 'everything'. Should you create separate APIs?",
      "opts": [
        "Yes—separate APIs for each client",
        "No—one API can return different data based on query parameters or use GraphQL to let clients specify exactly what they need. Don't duplicate logic.",
        "Create separate servers",
        "Mobile should use web API"
      ],
      "correct": 1,
      "explain": "One API, flexible responses. REST: Use query params like ?fields=name,price. GraphQL: Let clients query exactly what they need. Separate APIs = duplicate logic, more maintenance."
    },
    {
      "q": "What's the difference between PUT and POST?",
      "opts": [
        "No difference",
        "POST creates new resource, PUT updates existing resource (or creates if you specify ID). POST to /products creates product. PUT to /products/123 updates product 123.",
        "PUT is faster",
        "POST is more secure"
      ],
      "correct": 1,
      "explain": "REST conventions: POST creates (generates new ID), PUT updates (you specify ID). POST /products creates new. PUT /products/123 updates existing. This convention helps API users understand what each endpoint does."
    },
    {
      "q": "Why do APIs need versioning?",
      "opts": [
        "To track changes",
        "Breaking changes don't break existing clients. Mobile apps can't force-update. Third-party integrations need stability. v1 works while v2 adds features.",
        "Makes APIs faster",
        "Required by standards"
      ],
      "correct": 1,
      "explain": "Version APIs to avoid breaking existing clients. If you change response format, old mobile apps break. Versioning lets old clients use v1 while new clients use v2. Critical for stability."
    },
    {
      "q": "Your API returns 50 fields but frontend uses only 5. Problem?",
      "opts": [
        "No problem—more data is better",
        "Wastes bandwidth and slows responses. Large payloads slow mobile users. Solution: Let clients specify needed fields (?fields=name,price) or use GraphQL.",
        "Need faster server",
        "Should use POST instead"
      ],
      "correct": 1,
      "explain": "Over-fetching wastes bandwidth. 50-field response when you need 5 = slow, especially on mobile. Solution: Query parameters to select fields, or GraphQL to request exactly what's needed."
    },
    {
      "q": "When should you choose GraphQL over REST?",
      "opts": [
        "Always—GraphQL is newer",
        "When you have multiple clients (web, mobile, third-party) with different data needs, or complex nested queries. Don't use GraphQL if REST works—adds complexity.",
        "Never—REST is standard",
        "Only for startups"
      ],
      "correct": 1,
      "explain": "GraphQL solves specific problems: flexible querying, avoiding over/under-fetching, multiple client types. But adds complexity (learning curve, caching harder). Use REST unless you have these specific problems."
    }
  ],
  "essay": {
    "q": "Your mobile team wants a separate API because 'the web API returns too much data'. Your architect proposes GraphQL. Your CTO wants to keep REST simple. How do you evaluate these options? What are the trade-offs?",
    "guide": "Current problem: Mobile uses 5 fields, web API returns 50. Options: 1) Separate mobile API: Duplicate logic, double maintenance, different bugs. Cost: High. Benefit: Mobile-optimized. 2) Add query params to REST: ?fields=name,price. Cost: Low (1-2 days). Benefit: Flexible, backward compatible. 3) GraphQL: Clients query exact needs. Cost: 2-4 weeks migration + learning curve. Benefit: Very flexible, solves problem long-term. Recommendation: Start with option 2 (query params). Measure if it solves problem. If mobile needs grow complex (nested data, many variations), consider GraphQL. Don't rewrite unless complexity justifies cost. Calculate: GraphQL migration = 4 weeks × 3 devs = £96k. Query params = 2 days × 1 dev = £1.6k. Start cheap, add complexity only if needed."
  }
};
